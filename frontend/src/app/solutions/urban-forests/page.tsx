    ...
    <main className="min-h-screen bg-transparent pt-20">
      ...
      <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 border-b border-divider">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold text-content mb-6">Not Just Generic Forestry</h2>
          <p className="text-content-muted text-xl font-light">We organize our urban forests around distinct possibilities and purposes.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {[
            ...
            <div key={i} className="bg-nabtura-slate border border-divider p-8 rounded-3xl hover:border-nabtura-green transition-colors">
              <h4 className="text-content font-bold text-lg mb-3 tracking-wide">{type.title}</h4>
              <p className="text-content-muted font-light text-sm">{type.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-24 bg-nabtura-slate border-b border-divider">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            <div>
              <h3 className="text-3xl md:text-5xl font-bold text-content mb-12">How NABTURA Designs It</h3>
                ...
                {[
                  ...
                  return (
                    <div key={i} className="flex gap-6 items-start group">
                      <div className="w-12 h-12 rounded-full bg-overlay backdrop-blur-xl border border-divider flex items-center justify-center shrink-0 group-hover:border-nabtura-green transition-colors">
                        <Icon className="w-5 h-5 text-content-muted group-hover:text-nabtura-green transition-colors" />
                      </div>
                      <div>
                        <h4 className="text-xl font-bold text-content mb-2">{step.title}</h4>
                        <p className="text-content-muted font-light leading-relaxed">{step.desc}</p>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="bg-overlay backdrop-blur-xl/50 p-12 rounded-[3rem] border border-divider flex flex-col justify-center">
              <h3 className="text-3xl font-bold text-content mb-6">Why Create One?</h3>
              <ul className="space-y-4 text-content-muted font-light text-lg mb-12">
