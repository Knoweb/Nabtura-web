...
async function update() {
  try {
    const res = await axios.get(`${STRAPI_URL}/api/possibilities`);
    const items = res.data.data;
    
    for (const item of items) {
      const name = item.attributes?.name || item.name; // depending on strapi 4 vs 5
      const id = item.id;
      const documentId = item.documentId;
      
      const newDesc = structuredData[name];
      if (newDesc) {
        // strapi 5 usually uses documentId for updates
        const updateUrl = documentId ? `${STRAPI_URL}/api/possibilities/${documentId}` : `${STRAPI_URL}/api/possibilities/${id}`;
